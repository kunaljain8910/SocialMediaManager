...
const T = {
  ...
  cardH:       "#131c2e",
  border:      "#1e2d4a",
  borderH:     "rgba(201,168,76,0.25)",
  primary:     "#c9a84c",
  primaryH:    "#b8952e",
  primaryGlow: "rgba(201,168,76,0.18)",
  blue:        "#3B82F6",
  cyan:        "#06B6D4",
  green:       "#10B981",
  amber:       "#e8c96a",
...
function StatCard({ label, value, sub, icon: Icon, color, trend }) {
  return (
    <Card style={{ padding:"1.25rem" }}>
      ...
      <div style={{ fontSize:24, fontWeight:700, color:T.text, letterSpacing:"-0.5px", marginBottom:3, fontFamily:"'Cormorant Garamond', serif" }}>{value}</div>
      ...
      <div style={{ padding:collapsed ? "20px 0" : "20px 20px", borderBottom:`0.5px solid ${T.border}`, display:"flex", alignItems:"center", justifyContent:collapsed ? "center" : "space-between" }}>
        {!collapsed && (
          <div style={{ display:"flex", alignItems:"center", gap:8 }}>
            <div style={{ width:32, height:32, borderRadius:8, background:`linear-gradient(135deg, ${T.primary}, #7a5f28)`, display:"flex", alignItems:"center", justifyContent:"center", boxShadow:`0 0 12px ${T.primaryGlow}` }}>
              <span style={{ fontSize:14, fontWeight:700, color:T.bg, fontFamily:"serif" }}>✦</span>
            </div>
            <div>
              <span style={{ fontSize:15, fontWeight:700, color:T.text, letterSpacing:"1px", fontFamily:"'Cormorant Garamond', serif" }}>RASAYA</span>
              <span style={{ fontSize:11, color:T.primary, marginLeft:5, fontWeight:600, letterSpacing:"1px" }}>SOCIAL</span>
            </div>
          </div>
        )}
        {collapsed && (
          <div style={{ width:32, height:32, borderRadius:8, background:`linear-gradient(135deg, ${T.primary}, #7a5f28)`, display:"flex", alignItems:"center", justifyContent:"center" }}>
            <span style={{ fontSize:14, fontWeight:700, color:T.bg, fontFamily:"serif" }}>✦</span>
          </div>
        )}
            ...
            <div style={{ marginTop:4, background:T.card, border:`0.5px solid ${T.border}`, borderRadius:10, overflow:"hidden" }}>
              {stores.map(s => (
                <div key={s.id} onClick={() => { setStore(s); setStoreOpen(false); }}
                  ...
                  <div style={{ width:22, height:22, borderRadius:5, background:s.color, display:"flex", alignItems:"center", justifyContent:"center", fontSize:9, fontWeight:700, color:T.bg }}>{s.avatar}</div>
      ...
      <div style={{ padding:collapsed ? "12px 8px" : "12px 12px", borderTop:`0.5px solid ${T.border}` }}>
        {collapsed ? (
          ...
          <div style={{ display:"flex", alignItems:"center", gap:10, padding:"8px 10px", borderRadius:10, cursor:"pointer" }}>
            <div style={{ width:30, height:30, borderRadius:"50%", background:`linear-gradient(135deg, ${T.primary}, #7a5f28)`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:12, fontWeight:700, color:T.bg }}>A</div>
            <div style={{ flex:1, minWidth:0 }}>
              <div style={{ fontSize:12, fontWeight:500, color:T.text }}>Admin</div>
              <div style={{ fontSize:11, color:T.sub }}>admin@rasaya.in</div>
            </div>
...
function Header({ title, store, actions }) {
  return (
    <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:24 }}>
      <div>
        <h1 style={{ fontSize:20, fontWeight:700, color:T.text, margin:0, letterSpacing:"0.5px", fontFamily:"'Cormorant Garamond', serif" }}>{title}</h1>
  ...
  return (
    <div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"2fr 1fr", gap:16, marginBottom:24 }}>
        ...
        <Card style={{ padding:"1.25rem" }}>
          <div style={{ fontSize:16, fontWeight:600, color:T.text, marginBottom:4, fontFamily:"'Cormorant Garamond', serif" }}>Platforms</div>
      ...
      <Card style={{ padding:"1.25rem", marginBottom:24 }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:16 }}>
          <div>
            <div style={{ fontSize:16, fontWeight:600, color:T.text, marginBottom:2, fontFamily:"'Cormorant Garamond', serif" }}>Weekly Reach</div>
    ...
    <div>
      ...
      <Card style={{ padding:"1.25rem", marginBottom:20 }}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:20 }}>
          ...
          <div style={{ fontSize:18, fontWeight:700, color:T.text, fontFamily:"'Cormorant Garamond', serif" }}>{MONTHS[month]} {year}</div>
          ...
          {Array(days).fill(null).map((_,i) => {
            ...
            return (
              <div key={d} onClick={() => setSelected(isSel ? null : d)}
                ...
                <div style={{ fontSize:12, fontWeight:isToday ? 700 : 400, color:isToday ? T.primary : T.sub, marginBottom:4 }}>{d}</div>
                <div style={{ display:"flex", flexDirection:"column", gap:2 }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:12, marginTop:16 }}>
        {[
          ...
          <Card key={label} style={{ padding:"1rem", textAlign:"center" }}>
            <div style={{ fontSize:28, fontWeight:700, color, marginBottom:4, fontFamily:"'Cormorant Garamond', serif" }}>{value}</div>
              ...
              {store.platforms.map(p => {
                ...
                return (
                  <div key={p} onClick={() => setPlatform(p)}
                    style={{ display:"flex", alignItems:"center", gap:6, padding:"7px 14px", borderRadius:9, cursor:"pointer", background:active ? `${plt.color}20` : T.dark, border:`1px solid ${active ? plt.color : T.border}`, fontSize:13, color:active ? plt.color : T.sub, transition:"all 0.15s", fontWeight:active ? 600 : 400 }}>
    ...
    <div>
      <Header title="Analytics" store={store} actions={
        <div style={{ display:"flex", gap:4, background:T.card, border:`0.5px solid ${T.border}`, borderRadius:9, padding:3 }}>
          {["1m","3m","6m","1y"].map(p => (
            <div key={p} onClick={() => setPeriod(p)} style={{ padding:"4px 12px", borderRadius:7, fontSize:12, cursor:"pointer", background:period===p ? T.primary : "transparent", color:period===p ? T.bg : T.sub, transition:"all 0.15s", fontWeight:period===p ? 700 : 400 }}>{p}</div>
          ))}
        </div>
      } />
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:16, marginBottom:24 }}>
        ...
        <StatCard label="Total Likes"     value={fmtNum(totalLikes)}    icon={Heart}         color={T.red}     trend={8.7}  />
        <StatCard label="Comments"        value={fmtNum(totalComments)} icon={MessageCircle} color={T.amber}   trend={5.2}  />
        <StatCard label="Avg Engagement"  value={`${store.engagement}%`} icon={TrendingUp}   color={T.primary} trend={2.1}  />
      </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16, marginBottom:16 }}>
        <Card style={{ padding:"1.25rem" }}>
          <div style={{ fontSize:16, fontWeight:700, color:T.text, marginBottom:4, fontFamily:"'Cormorant Garamond', serif" }}>Engagement Rate</div>
      ...
      <Card style={{ padding:"1.25rem" }}>
        <div style={{ fontSize:16, fontWeight:700, color:T.text, marginBottom:16, fontFamily:"'Cormorant Garamond', serif" }}>Top Performing Posts</div>
    ...
    <div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"2fr 1fr", gap:20 }}>
        <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
          ...
          <Card style={{ padding:"1.5rem" }}>
            <div style={{ fontSize:16, fontWeight:700, color:T.text, marginBottom:16, fontFamily:"'Cormorant Garamond', serif" }}>Connected Platforms</div>
            <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
          <Card style={{ padding:"1.5rem" }}>
            <div style={{ fontSize:16, fontWeight:700, color:T.text, marginBottom:4, fontFamily:"'Cormorant Garamond', serif" }}>Current Plan</div>
            ...
            <Btn style={{ width:"100%", justifyContent:"center", marginTop:12, background:`linear-gradient(135deg, ${T.primary}, #7a5f28)`, border:"none" }}>
              Upgrade to Agency
            </Btn>
          </Card>

          <Card style={{ padding:"1.5rem" }}>
            <div style={{ fontSize:16, fontWeight:700, color:T.text, marginBottom:12, fontFamily:"'Cormorant Garamond', serif" }}>Notifications</div>
            {[["Post published","When posts go live"],["Weekly report","Every Monday 9 AM"],["Engagement alerts","When post goes viral"],["New followers","Daily summary"]].map(([label, desc]) => (
              <div key={label} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"8px 0", borderBottom:`0.5px solid ${T.border}` }}>
                ...
                <div style={{ width:36, height:20, borderRadius:10, background:T.primary, cursor:"pointer", position:"relative" }}>
                  <div style={{ position:"absolute", right:2, top:2, width:16, height:16, borderRadius:8, background:T.bg }} />
                </div>
              </div>
            ))}
          </Card>
    ...
    <div style={{ display:"flex", height:"100vh", background:T.bg, color:T.text, fontFamily:"'Outfit', 'Segoe UI', system-ui, sans-serif", overflow:"hidden" }}>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        input::placeholder, textarea::placeholder { color: #3d4f6a; }
